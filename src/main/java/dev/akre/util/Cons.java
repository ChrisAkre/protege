...
public record Cons<T>(T head, Cons<T> tail) implements UnmodifiableCons<T> {
    ...
    /**
     * The singleton nil (empty list) instance.
     */
    public static final Cons<?> NIL = new Cons<>(null, null);

    /**
     * Compact constructor that validates that the cons cell contains non-null components,
     * unless it is the special NIL instance.
     *
     ...
     * @return A new Cons with {@code t} as the head and this list as the tail.
     */
     ...
     * @param <T>    The type of elements.
     * @return A Cons list containing the values.
     */
