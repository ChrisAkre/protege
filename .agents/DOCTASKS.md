...
- `ProtoCodegen` is located in `dev.akre.protege.compiler` package, not the root package.

## Pending Approval
- None.

## Approved
- [x] Fix Javadoc warnings in `CodegenConfig.java` (missing params/returns).
- [x] Fix Javadoc warnings in `CodegenUtils.java`, `EnumCodegen.java`, `Field.java`.
- [x] Fix Javadoc warnings in `dev.akre.util` package (`Cons`, `UnmodifiableCons`).
- [x] Update `AGENTS.md` with correct source paths.
