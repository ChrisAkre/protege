...
- `ProtoCodegen.java` is marked for removal/renaming to `CodegenMetadata`.

## Pending Approval
- None.

## Approved
- [ ] Optimize `ProtoUtils.indent` and `messageToString` to avoid repeated string manipulation (see TODO in `ProtoUtils.java`).
...
- [x] Document "Dark Zones": `ProtoCodegen.java` - Logic overlap with `CodegenMetadata`.
