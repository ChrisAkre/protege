# Oliver's Documentation Tasks

## Memories
* Project is a Java-based code generation library for Protocol Buffers.
* Uses Maven, ANTLR v4, and JavaPoet.
* Documentation should be clear, minimal, and machine-friendly.

## Pending Approval

## Approved
- [ ] Explore codebase for TODOs, FIXMEs, and commented-out code.
- [ ] Check for missing Javadocs in complex methods.
- [ ] Verify `README.md` instructions.
- [ ] Run `mvn javadoc:javadoc` to check for existing errors.
